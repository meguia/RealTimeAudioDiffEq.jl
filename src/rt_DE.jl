...
function _channel_map_from_vector(indices::AbstractVector{<:Integer}, n_vars::Int)::Matrix{Float64}
	n_channels = length(indices)
	channel_map = zeros(Float64, n_vars, n_channels)
	for (channel, variable) in enumerate(indices)
		if variable < 1 || variable > n_vars
			error("channel_map variable index out of bounds.")
		end
		channel_map[variable, channel] = 1.0
	end
	return channel_map
end

function _channel_map_from_vector(indices_per_channel::AbstractVector{<:AbstractVector{<:Integer}}, n_vars::Int)::Matrix{Float64}
	n_channels = length(indices_per_channel)
	channel_map = zeros(Float64, n_vars, n_channels)
	for (channel, variables) in enumerate(indices_per_channel)
...
function normalize_channel_map(channel_map::AbstractMatrix{<:Real}, n_vars::Int)::Matrix{Float64}
	return Matrix{Float64}(channel_map)
end

function normalize_channel_map(channel_map::AbstractVector{<:Integer}, n_vars::Int)::Matrix{Float64}
	return _channel_map_from_vector(channel_map, n_vars)
end

function normalize_channel_map(channel_map::AbstractVector{<:AbstractVector{<:Integer}}, n_vars::Int)::Matrix{Float64}
	return _channel_map_from_vector(channel_map, n_vars)
end
...
"""
    DESource(f, u0::Vector{Float64}, p::Vector{Float64};
		alg = Tsit5(), channel_map = [1, 1])::DESource
...
- `channel_map`: accepted formats:
	- `Matrix{<:Real}` with size `(nvars, nchannels)` where rows are variables and columns are output channels.
	- `Vector{Int}` where each entry is the variable index for that channel (e.g. `[1, 2]`).
	- `Vector{Vector{Int}}` where each inner vector lists variable indices mixed into each channel.
"""
...
function DESource(f, g, u0::Vector{Float64}, p::Vector{Float64};
		alg = SOSRA(), channel_map = [1, 1])::DESource
...
function _DESource(prob, alg, channel_map)::DESource
	n_vars = length(prob.u0)
	normalized_channel_map = normalize_channel_map(channel_map, n_vars)

	control = RealTimeAudioDEControlData(prob.u0, prob.p, 1., 1., normalized_channel_map)
...
#! export
"""
    get_param(source::DESource, index::Int)
...
#! export
"""
    set_channelmap!(source::DESource, channel_map)
Set the channel map of the DESource.

`channel_map` accepts the same formats as the constructor and is normalized internally
to a matrix with size `(nvars, nchannels)`.
"""
function set_channelmap!(source::DESource, channel_map)
	# check variables
	n_vars = length(source.data.problem.u0)::Int
	normalized_channel_map = normalize_channel_map(channel_map, n_vars)
	
	if size(normalized_channel_map, 1) > n_vars
		@warn "$(size(normalized_channel_map, 1) - n_vars) variable(s) out of bounds."
	end
		
	status = Pa_IsStreamActive(source.data.stream_data.stream[])
	if status == 1 # stream is running
		# check number of channels
		n_channels_in_map = size(normalized_channel_map, 2)
	...
	@atomic source.data.control.channel_map = normalized_channel_map
end
